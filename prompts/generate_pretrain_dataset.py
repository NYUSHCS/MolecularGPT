...
parser.add_argument('--generate_qm9_text',action='store_true')
...
if args.split_non_overlap:
    if args.generate_qm9_text:
        print(f'generate qm9 non-overlap data with {args.split_file}')
        args.split_file = 'qm9_non_overlap_' + args.split_file
    else:
...
def load_split_file(mol_smiles,assays):
    ...
    if args.split_file is not None and os.path.exists(split_file_dir):
        ...
        train_assays = ['homo','lumo']
        valid_assays = ['gap']

        if args.split_non_overlap:
            train_graph_index = np.random.choice(np.arange(133885), size=int(133885 * 0.8),
                                                 replace=False)
            valid_graph_index = np.delete(np.arange(133885), train_graph_index)
        else:
            train_graph_index = np.arange(133885)
            valid_graph_index = np.arange(133885)
...
def generate_assay_data(assay_ids, molecule_index_set,split_name):
    ...
    for _, assay_chembl_id in tqdm(enumerate(assay_ids)):
        ...
        df.to_parquet(os.path.join('./train_process/assay', assay_chembl_id, assay_chembl_id +'.parquet'))
...
def generate_property_data(molecule_index_set,split_name):
    prefix ='Molecule_molecules_molecule_properties_'
    a = len(prefix)
    data_all = []
                ...
                for key in data_mol_property.columns:
                    ...
                    if not (key in [] or pd.isnull((properties[key])) or properties[key] in ['[]', []] or (
                        ...
                        if key_prompt in prompts['chembl_property']:
                            ...
                            data_all.append((graph_data, text_prompt, label, graph_chembl_id, shot_key))
                        ...
                        if add_negation:
                            df.to_parquet(os.path.join('./train_process/property', key+'_negative', key+'_negative.parquet'))
                        else:
                            df.to_parquet(os.path.join('./train_process/property', key, key+'.parquet'))
...
def generate_qm9_data(assay_ids, molecule_index_set,split_name, qm9_data):

    data_all = []
    graph_chembl_id = []
    for _, graph_id in tqdm(enumerate(molecule_index_set)):
        graph_chembl_id = qm9_data['mol_id'][graph_id]
        properties = data_mol_property.loc[graph_chembl_id]
        graph_data = properties['graph']
        for _, key in tqdm(enumerate(assay_ids)):
            if not (key in [] or pd.isnull((properties[key])) or properties[key] in ['[]', []] or (
                    properties[key] is None)):

                if key in prompts['qm9']:
                    if args.augmented_type is not None:
                        text_prompts = prompts_augmented['qm9'][key][args.augmented_type]
                        prompt_id = np.random.randint(0, len(text_prompts))
                        text_prompt = text_prompts[prompt_id]
                    else:
                        text_prompts = prompts['qm9'][key]
                        
                        prompt_id = np.random.randint(0, len(text_prompts))
                        text_prompt = text_prompts[prompt_id]
                    label = map_label(properties[key], column_type[key])

                    data_all.append((graph_data, text_prompt, label, graph_chembl_id, key))
                    ...
                    df.to_parquet(os.path.join('./train_process/qm9', key, key+'.parquet'))
    
    # file_prefix = "qm9_graph_text_"
    # if args.augmented_type is not None:
    #     file_prefix += args.augmented_type + '_'
    # df.to_csv(os.path.join('..', 'pretrain_datasets', file_prefix + split_name+ '_' + args.split_file + args.file_save_name))
...
if __name__ == '__main__':
    ...
    if args.generate_qm9_text:
        ...
        data_mol_property=pd.read_csv(os.path.join('..','prompt_data','qm9.csv'),header=0)
        qm9_data  = data_mol_property
        data_mol_property=data_mol_property.set_index('mol_id')
        column_type={}
        ...
        generate_qm9_data(train_assays,set(train_graph_index),'train', qm9_data)
        generate_qm9_data(valid_assays,set(valid_graph_index),'valid', qm9_data)

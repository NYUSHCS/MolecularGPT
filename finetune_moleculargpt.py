...
# cached_assets_path(library_name="datasets", assets_dir="/gpfsnyu/scratch/jd5849/.cache/huggingface")
...
def main():
    ...
    def tokenize(prompt, add_eos_token=True):
        ...
        if (
            ...
            # right append, append eos, end of a input
    ...
    def generate_and_tokenize_prompt(data_point):
        ...
        if not data_args.train_on_inputs:
            # print(f'Not train on input!')
            user_prompt = prompter.generate_prompt(
                data_point["instruction"], data_point["input"]
            )
            tokenized_user_prompt = tokenize(user_prompt, add_eos_token=data_args.add_eos_token)
    ...
    if data_args.train_files[0].endswith(".json") or data_args.train_files[0].endswith(".jsonl"):
        raw_datasets_train = load_dataset(
            "json", 
            data_files=data_args.train_files[0],
        ...
        raw_datasets_train = load_dataset(
            data_args.train_files[0],
            cache_dir=os.path.join(training_args.output_dir,'dataset_cache'),
            use_auth_token=True if model_args.use_auth_token else None,
            # **dataset_args,
            )  
  
    if data_args.validation_files[0].endswith(".json") or data_args.validation_files[0].endswith(".jsonl"):

        raw_datasets_val = load_dataset(
            "json", 
            data_files=data_args.validation_files[0],
            cache_dir=os.path.join(training_args.output_dir,'dataset_cache'),
            use_auth_token=True if model_args.use_auth_token else None,
            # **dataset_args,
            )
        ...
        raw_datasets_val = load_dataset(
            data_args.validation_files[0],
            cache_dir=os.path.join(training_args.output_dir,'dataset_cache'),
            use_auth_token=True if model_args.use_auth_token else None,
            # **dataset_args,
            ) 
      
    train_data = raw_datasets_train["train"].shuffle().map(
        generate_and_tokenize_prompt,
        batched= False,
        num_proc=data_args.preprocessing_num_workers,
        remove_columns=list(raw_datasets_train["train"].features),
        load_from_cache_file=not data_args.overwrite_cache,
        desc="Running tokenizer on dataset",
    )
    val_data = raw_datasets_val["train"].shuffle().map(
        generate_and_tokenize_prompt,
        batched= False,
        num_proc=data_args.preprocessing_num_workers,
        remove_columns=list(raw_datasets_val["train"].features),
        load_from_cache_file=not data_args.overwrite_cache,
        desc="Running tokenizer on dataset",
        )
